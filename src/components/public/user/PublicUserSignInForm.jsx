...
//Todo: GraphQl
import { SIGN_IN_MUTATION, SIGN_IN_SOCIAL_MUTATION } from '../../../graphql/mutations/user_mutation'
...
class PublicUserSignInForm extends PureComponent {
    ...
    responseFacebook = async (response, action, social) => {
        if (response) {
            const signInfo = {
            ...
            const status = await action({ variables: { signInfoSocialData: signInfo } });
            if (status != null && status != '') {
                const userInfo = jwt.decode(status.data.signInSocial.jwt);
                userInfo["jwt"] = status.data.signInSocial.jwt;
                this.setLocalUserData(userInfo);
            }
        }
    }
    validEmail = (e, text) => {
    ...
    signIn = async (e, action) => {
        ...
        if (status != null && status != '') {
            ...
            userInfo["jwt"] = status.data.signIn.jwt;
            this.setLocalUserData(userInfo);
        }
    }
    setLocalUserData = async (userInfo) => {
                    ...
                    <div className={signStyles.signForm} id="sign-in-form">
                        ...
                        <Grid container className={signStyles.buttonDiv}>
                            ...
                            <Grid item xs={6} className={`${signStyles.socialSignIn}`}>
                                <Mutation mutation={SIGN_IN_SOCIAL_MUTATION} onError={(error) => this.signInErrorHandler(error)}>
                                    {(action, { data, loading, error }) => (<Fragment>
                                        <FacebookLogin
                                            appId="2228743917353964"
                                            fields="name,email,picture.width(500).height(500)"
                                            callback={(response) => this.responseFacebook(response, action, 'FACEBOOK')}
                                            render={renderProps => (
                                                <Button className={`${appStyles.buttonSvg} ${signStyles.socialSignIn}`} onClick={renderProps.onClick}>
                                                    {renderProps.isProcessing || loading ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className={signStyles.svgLoadingBlockTime}>
                                        ...
                                        <GoogleLogin
                                            clientId="932888515281-dkdchndqi2romejqhcrvdu23mfch0qnu.apps.googleusercontent.com"
                                            onSuccess={(response) => this.responseFacebook(response, action, 'GOOGLE')}
                                            render={renderProps => (
                                                <Button className={`${appStyles.buttonSvg} ${signStyles.socialSignIn}`} onClick={renderProps.onClick}>
                                                    {loading ? <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className={signStyles.svgLoadingBlockTime}>
                                                        <path transform="translate(2)" d="M0 12 V20 H4 V12z">
                                                            <animate attributeName="d" values="M0 12 V20 H4 V12z; M0 4 V28 H4 V4z; M0 12 V20 H4 V12z; M0 12 V20 H4 V12z" dur="1.2s" repeatCount="indefinite" begin="0" keyTimes="0;.2;.5;1" keySplines="0.2 0.2 0.4 0.8;0.2 0.6 0.4 0.8;0.2 0.8 0.4 0.8" calcMode="spline" />
                                                        </path>
                                                        <path transform="translate(8)" d="M0 12 V20 H4 V12z">
