...
export const PrivateIndexTopMemberChildForm = React.memo((props) => {
    ...
    return <Fragment>
        <div className={`${topMemberStyles.userInfoDiv} ${user.status == 'ON' ? topMemberStyles.userOnline:null}`} onMouseOver={() => setShowOption(true)} onMouseLeave={() => setShowOption(false)}>
            ...
            <div className={topMemberStyles.divStatus}>
                <label className={topMemberStyles.profileName}>{user.profileName}</label>
                {user.status == 'ON' && <span className={topMemberStyles.statusSpan}>
                    <svg viewBox="0 0 24 24">
                        <path fill="#000000" d="M19,12C19,15.86 15.86,19 12,19C8.14,19 5,15.86 5,12C5,8.14 8.14,5 12,5C15.86,5 19,8.14 19,12Z" />
                    </svg>
                    <label className={topMemberStyles.statusLabel}>online</label>
                </span>}
            </div>
